...
*/
isOAuthModal = true;
const isBotMode = false;
// const isTelemetryDisabled = true;
const isDebug = true;
...
if (document.location.pathname.includes("mini.html")) {
  routerBaseName = "/";
}
...
async function updateEnvironmentForHub(hub, entryManager) {
  console.log("updateEnvironmentForHub");
  let sceneUrl;
  let isLegacyBundle; // Deprecated

  const sceneErrorHandler = () => {
    console.log("sceneerror", ExitReason.sceneError);
...
function handleHubChannelJoined(entryManager, hubChannel, messageDispatch, data) {
  ...
  const objectsUrl = "localhost/objects.gltf";
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  */
  ...
  const authChannel = null;
  // const hubChannel = new HubChannel(store, hubId);
  const hubChannel = null;
  ...
  */
  ...
  // tk const socket = await connectToReticulum(isDebug);
  const socket = null;
